...
class SendUseCase {
    ...
    async execute(data) {
        const comunication = await this.comunicationRepository.findBy("id", data.id);
        if (!comunication)
            throw new Error("Comunication not found");
        const extracts = (await (0, get_template_form_data_1.getTemplateFormData)()).filter((item) => item.accept_email && item.degree.includes(Degree[data.degree]));
